...
import { ICustodian, IERC20, IExchange, ILiquidityProviderToken, IWETH9 } from './libraries/Interfaces.sol';
import { Asset, HybridTrade, LiquidityAddition, LiquidityAdditionDepositResult, LiquidityChangeExecution, LiquidityMigration, LiquidityPool, LiquidityRemoval, LiquidityRemovalDepositResult, NonceInvalidation, Order, OrderBookTrade, PoolTrade, Withdrawal } from './libraries/Structs.sol';
...
contract Exchange_v3_1 is IExchange, Owned {
  ...
  function depositTokenBySymbol(
    ...
    Asset memory asset = _assetRegistry.loadAssetBySymbol(
      assetSymbol,
  ...
  function addLiquidity(
    ...
    LiquidityAdditionDepositResult memory result = _liquidityPools.addLiquidity(
      LiquidityAddition(
  ...
  function removeLiquidity(
    ...
    LiquidityRemovalDepositResult memory result = _liquidityPools
      .removeLiquidity(
        LiquidityRemoval(
          Constants.signatureHashVersion,
          LiquidityChangeOrigination.OnChain,
  ...
  function withdrawExit(address assetAddress) external {
    ...
    uint64 previousExchangeBalanceInPips = Withdrawing.withdrawExit(
      assetAddress,
      _custodian,
      _assetRegistry,
      _balanceTracking
