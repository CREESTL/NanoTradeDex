...
import { HybridTrade, Order, OrderBookTrade, NonceInvalidation, PoolTrade } from './Structs.sol';
...
library Trading {
  ...
  function executePoolTrade(
    ...
    bytes32 orderHash = PoolTradeValidations.validatePoolTrade(
      order,
      poolTrade,
      assetRegistry,
      nonceInvalidations
  ...
  function executeHybridTrade(
    ...
    (bytes32 buyHash, bytes32 sellHash) = HybridTradeValidations
      .validateHybridTrade(
        buy,
        sell,
        hybridTrade,
    ...
    {
      address takerWallet = hybridTrade.orderBookTrade.makerSide ==
        OrderSide.Buy
        ? sell.walletAddress
        : buy.walletAddress;
