...
import { ICustodian, IERC20, IExchange, ILiquidityProviderToken, IWETH9 } from './Interfaces.sol';
...
library LiquidityPoolAdmin {
  ...
  function upgradeLiquidityPool(
    ...
    LiquidityPool memory migrationSourcePool = balanceMigrationSource
      .loadLiquidityPoolByAssetAddresses(baseAssetAddress, quoteAssetAddress);
    ...
    Asset memory lpTokenAsset = Asset({
      exists: true,
  ...
  function reverseLiquidityPoolAssets(
    ...
    LiquidityPool memory pool = LiquidityPools
  ...
  function createLiquidityPoolByAssetAddresses(
    ...
    ILiquidityProviderToken liquidityProviderToken = new LiquidityProviderToken{
      salt: salt
    ...
    )
  {
    ...
    address unwrappedToken0 = migration.token0 == address(migration.WETH)
      ? address(0x0)
      : migration.token0;
    address unwrappedToken1 = migration.token1 == address(migration.WETH)
      ? address(0x0)
      : migration.token1;
