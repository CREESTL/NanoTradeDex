...
import { Asset, LiquidityPool, Order, OrderBookTrade, NonceInvalidation, PoolTrade, Withdrawal } from './Structs.sol';
...
library Validations {
   ...
   *
   */
  function validateOrderBookTradeFees(
  ...
  function validatePoolReserveRatio(LiquidityPool memory pool) internal pure {
    (uint64 sortedReserve0, uint64 sortedReserve1) = pool
      .baseAssetReserveInPips <= pool.quoteAssetReserveInPips
  ...
  function validatePoolTradeInputFees(
    ...
    (
      uint64 netInputQuantityInPips,
      uint64 grossInputQuantityInPips
