...
library AssetRegistry {
  ...
  function loadBalanceInAssetUnitsBySymbol(
    ...
    Asset memory asset = loadAssetBySymbol(
      self,
      assetSymbol,
  ...
  function loadBalanceInPipsBySymbol(
    ...
    address assetAddress = loadAssetBySymbol(
      self,
      assetSymbol,
      getCurrentTimestampInMs()
    ).assetAddress;
  ...
  function loadBalanceInPipsFromMigrationSourceIfNeeded(
    ...
    BalanceTracking.Balance memory balance = balanceTracking
      .balancesByWalletAssetPair[wallet][assetAddress];
  ...
  ) private pure returns (Asset memory) {
