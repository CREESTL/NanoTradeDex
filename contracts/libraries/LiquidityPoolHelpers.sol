...
library LiquidityPoolHelpers {
  function calculateCurrentPoolPriceInPips(
  ...
  {
    uint64 totalLiquidityInPips = AssetUnitConversions.assetUnitsToPips(
    ...
    uint256 poolPriceInDoublePips = (uint256(self.quoteAssetReserveInPips) *
      Constants.doublePipPriceMultiplier) / self.baseAssetReserveInPips;
    uint64 targetQuoteAssetReserveInPips = SafeCast.toUint64(
  ...
  function calculateOutputLiquidityInPips(
    ...
    uint256 totalSupplyInAssetUnits = IERC20(
      address(self.liquidityProviderToken)
    ).totalSupply();
