...
contract("OrdersController", ([owner, alice, bob]) => {
    beforeEach(async () => {
        ...
        this.dai = await Mock.new("DAI", "DAI", ether("1000000"), {
            from: bob,
        });
    ...
    describe("check orderController token balance update", async () => {
        ...
        it("should update tokenBalance mapping", async () => {
            ...
            expect(
                await this.orderController.getAccumulatedFeeBalance(
                    this.weth.address
                )
            ).to.be.bignumber.equal(ether("0"));
    ...
    describe("Check matchOrders function", async () => {
        ...
        it("check correct fee in events", async () => {
            ...
            expect(matchHash.logs[1].args.fee).to.be.bignumber.equal(
                ether("0.25")
            );
            expect(matchHash.logs[2].args.fee).to.be.bignumber.equal(
                ether("0.25")
            );
            expect(matchHash.logs[3].args.fee).to.be.bignumber.equal(
                ether("0.125")
            );
            expect(matchHash.logs[4].args.fee).to.be.bignumber.equal(
                ether("0.625")
            );
    ...
    describe("check cancel function", async () => {
        it("should cancel order", async () => {
            ...
            expect((await this.orderController.getOrderInfo(id1))[8]).to.equal(
                false
            );
    ...
    describe("check userId mapping update", async () => {
        it("should cancel order", async () => {
            ...
            expect(
                (
                    await this.orderController.getUserOrderIds(0, 1000, {
                        from: alice,
                    })
                ).length
            ).to.equal(1);
    ...
    describe("order overmatching", () => {
        it("should not match with additional orders if order finished", async () => {
            ...
            console.log(
                "OrderUpdated events count:",
                orderUpdatedEvents.length
            );
            await processEventsArgs(
                match.receipt,
                "OrderUpdated",
                async (args) => {
    ...
    describe.only("empty orders", () => {
        it("should reproduce bug", async () => {
            ...
            await this.orderController.cancelOrder(matchingOrderID, {
                from: alice,
            });
